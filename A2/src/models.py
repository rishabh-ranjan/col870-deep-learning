...
class LeNetDropout(nn.Module):
    def __init__(self, num_classes=9):
        ...
        self.fc = nn.Sequential(
            nn.Linear(256,120), nn.ReLU(), nn.Dropout(0.5),
            nn.Linear(120,84), nn.ReLU(),
            nn.Linear(84,num_classes)
        )
    ...
    def forward(self, X):
        return self.fc(self.dropout(torch.flatten(self.conv(X),-3,-1)))
...
class LeNetRRN(nn.Module):
    def __init__(self, n_steps, w1, w2):
        super().__init__()
        self.lenet = LeNetDropout(num_classes=9)
    ...
    def criterion(self, X_true, aug_X, aug_y):
        ...
        self.pos_loss = self.rrn.criterion(Y=None, P=P)
