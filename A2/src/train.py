...
def train_rrn(net, X, Y, lr, batch_size, n_epochs, device, steps=None, show_step=None):
    ...
    #sched = optim.lr_scheduler.LambdaLR(opt, lr_lambda=lmbd)
...
def train_rrn_val(net, X, Y, val_X, val_Y, lr, batch_size, n_epochs, device, steps, show_step=None):
    ...
    for epoch in tqdm(range(n_epochs), 'epochs'):
        for XY, val_XY in zip(tqdm(loader, 'batches', leave=False), it.cycle(val_loader)):
            ...
            if ctr % 50 == 0:
                print(f'Epoch: {epoch + 1}, iteration: {ctr}, train loss: {step_losses[-1][-1]}, val loss: {val_losses[-1]}')
...
def train_lenet_rrn(net, loader, aug_loader, test_X, test_Y, test_x, test_y, lr, n_epochs, device, validation=False, show_step=None):
    ...
    for epoch in tqdm(range(n_epochs), 'epochs'):
        for XY, aug_Xy in zip(tqdm(loader, 'batches', leave=False), aug_loader):
            ...
            if ctr % 50 == 0:
                print(f'Epoch: {epoch + 1}, iteration: {ctr}, train loss: {pos_losses[-1]}')
           
            if validation and show_step is not None and ctr % show_step == 0:
                with plt_out:
                    plt.figure()
